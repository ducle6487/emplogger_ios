...
@propertyWrapper
public class InjectPublished<Dependency>: DependencyLifecycleScope {
    ...
    public var projectedValue: Wrapper {
        ...
        /// our single reference of `InjectPublished` and its single `Publisher`.
        ///
    ...
    public struct Wrapper {
        private var wrapped: InjectPublished
        
        internal init(_ wrap: InjectPublished<Dependency>) {
            self.wrapped = wrap
        }
    ...
    /// A simple publisher wrapper for properties marked with the `@InjectPublished` attribute.
